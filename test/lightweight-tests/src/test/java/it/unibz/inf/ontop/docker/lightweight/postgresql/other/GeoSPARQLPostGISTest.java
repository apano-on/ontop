...
//    private static final String PROPERTIES_FILE = "/geospatial/postgis/geospatial-postgis.properties";
    protected static final String OWL_FILE = "/geospatial/citygml2.0.owl";
    protected static final String OBDA_FILE = "/geospatial/citygml2.0.obda";
    private static final String PROPERTIES_FILE = "/geospatial/citygml2.0.properties";
    ...
    public void testAskIntersects6() throws Exception {
        String query = "PREFIX : <http://ex.org/> \n" +
                "PREFIX geo: <http://www.opengis.net/ont/geosparql#>\n" +
                "PREFIX geof: <http://www.opengis.net/def/function/geosparql/>\n" +
                "ASK WHERE {\n" +
                ":1 a :Geom; geo:asWKT ?xWkt.\n" +
                "FILTER (geof:sfIntersects(?xWkt, '<http://www.opengis.net/def/crs/OGC/1.3/CRS84> POLYGON Z ((3 3 1, 8 3 1, 8 6 1, 3 6 1, 3 3 1))'^^geo:wktLiteral))\n" +
                "}\n";
        boolean val = executeAskQuery(query);
        Assertions.assertTrue(val);
    }
    ...
    public void testAskIntersectHasGeometry() throws Exception {
        String query = "PREFIX : <http://ex.org/> \n" +
                ...
                "BIND (geof:sfIntersects(?xWkt, ?bWkt) AS ?v)\n" +
                "}\n";
        executeAndCompareValues(query, ImmutableList.of());
    }
    ...
    public void testAskIntersectBuffer() throws Exception {
        String query = "PREFIX : <http://ex.org/> \n" +
                ...
                "}\n";
        executeAndCompareValues(query, ImmutableList.of(
                "\"POLYGON((7.061002551206959 48.78897147461564,6.922049938911179 48.179326183965884," +
    ...
    public void testAskUnion() throws Exception {
        String query = "PREFIX : <http://ex.org/> \n" +
                ...
                "}\n";
        executeAndCompareValues(query, ImmutableList.of(
                "\"MULTIPOINT(2.2945 48.8584,-0.0754 51.5055)\"^^<http://www.opengis.net/ont/geosparql#wktLiteral>"));
    }
    ...
    public void testAskUnionBuffer() throws Exception {
        String query = "PREFIX : <http://ex.org/> \n" +
                ...
                "}\n";
        executeAndCompareValues(query, ImmutableList.of(
                "\"GEOMETRYCOLLECTION(POINT(2.2945 48.8584),POLYGON((-0.075256104751844 51.50549640923696," +
    ...
    public void testSelectIntersection3() {
        String query = "PREFIX : <http://ex.org/> \n" +
                ...
                "\n" +
                "SELECT * WHERE {\n" +
                "?x a :Geoms; geo:asWKT ?xWkt.\n" +
                "?x rdfs:label ?xname .\n" +
                "?y a :Geoms2; geo:asWKT ?yWkt.\n" +
                "?y rdfs:label ?yname .\n" +
                "BIND(geof:buffer(?xWkt, 20, uom:metre) AS ?zWkt) .\n" +
                "FILTER(?xname = ?yname) .\n" +
                "BIND(geof:intersection(?zWkt, ?yWkt) as ?v) .\n" +
    ...
    public void Test123() {
        String query = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
                ...
                "SELECT ?v {\n" +
                "VALUES ?highwayclasses { lgdo:UnclassifiedHighway } .\n" +
                "?osmentity rdf:type ?highwayclasses .\n" +
                ...
                "BIND(geof:intersection(?impactArea, ?citygmlGeom) AS ?v) .\n" +
                "} LIMIT 2";
